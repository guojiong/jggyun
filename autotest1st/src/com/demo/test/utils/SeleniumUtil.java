...
public class SeleniumUtil {
	...
	public Alert switchToPromptedAlertAfterWait(long waitMillisecondsForAlert) throws NoAlertPresentException {
		...
		for (long i = 0; i < 1511; i += ONE_ROUND_WAIT) {
	 ...
	 * 包装点击Parent
	 */
	public void clickParent(By byElement) {

		try {
			clickTheClickableParent(byElement, System.currentTimeMillis(), 2500);
		} catch (StaleElementReferenceException e) {
			logger.error("The element you clicked:[" + byElement + "] is no longer exist!");
			Assert.fail("The element you clicked:[" + byElement + "] is no longer exist!");
		} catch (Exception e) {
...
//		s.selectByVisibleText(text);
		List<WebElement> uls = findElementsBy(by);
		if(uls.size() == 0) {
			logger.info("下拉选项为空，选择 [" + text + "] 失败");
			Assert.fail("下拉选项为空，选择 [" + text + "] 失败");
			return;
		}
	 ...
	 */
	public void selectIsOrNo(By by, String IsOrNo) {
		WebElement element = findElementBy(by);
		if(!IsOrNo.equals(element.getText())) {
			element.click();
		}
	}
	...
	/** 不能点击时候重试点击操作 */
	private void clickTheClickableParent(By byElement, long startTime, int timeOut) throws Exception {
		try {
			findElementBy(byElement).findElement(By.xpath("./..")).click();
		} catch (Exception e) {
			if (System.currentTimeMillis() - startTime > timeOut) {
				logger.warn(byElement+ " is unclickable");
				throw new Exception(e);
    ...
    public String httpPostWithJson(String url, String json) {
    	...
    	try {
    		...
    		httpPost.setEntity(requestEntity);
    		 ...
    		 logger.info("Start call interface");
    		//第四步发送HTTPPost请求，获取返回值
    		returnValue = httpClient.execute(httpPost, responseHandler); //调接口获取返回值时，必须用此方法
    	}catch (Exception e) {
    		logger.info("Interface call failed");
            Assert.fail("Interface call failed");
		}
    	finally {
    		try {
    			httpClient.close();
    		}catch (Exception e) {
    			logger.info("httpClient close failed");
                Assert.fail("httpClient close failed");
			}
    	}
    	
    	if(returnValue.contains("\"success\":false")) {
    		logger.info("httpClient close failed" + returnValue);
    		Assert.fail("httpClient close failed" + returnValue);
    	}
		return returnValue;
	}
     ...
     * @param tableDatas
     */
    public void openEditWin(String testData, List<WebElement> tableDatas) {
    	if(tableDatas.size()==0) {
    		logger.info("The list of data is empty！");
    		Assert.fail("The list of data is empty！");
    	}
    	String rowid = getRowId(testData, tableDatas);
    	if("".equals(rowid)) {
    		logger.info("Not found the specified data, testData = " + testData);
    		Assert.fail("Not found the specified data, testData = " + testData);
    	}
    	List<WebElement> tableBtns = this.findElementsBy(SuperAction.getLocateWay("xpath", "//div[@class='ag-pinned-right-cols-container']/div"));
    	for (WebElement element : tableBtns) {
			if(rowid.equals(element.getAttribute("row-id"))) {
				element.findElement(By.xpath(".//a[contains(text(),'编辑')]")).click();
			}
		}
	}
     ...
     * @param tableDatas
     */
    public void deleteData(String testData, List<WebElement> tableDatas) {
    	if(tableDatas.size()==0) {
    		logger.info("The list of data is empty！");
    		Assert.fail("The list of data is empty！");
    	}
    	...
    	for (WebElement element : tableBtns) {
			if(rowid.equals(element.getAttribute("row-id"))) {
				element.findElement(By.xpath(".//a[contains(text(),'删除')]")).click();
			}
		}
    }
    
    /**
     * 获取表中指定数据的rowid,并返回rowid
     * @param testData
     * @param tableDatas
     * @return
     */
    public String getRowId(String testData, List<WebElement> tableDatas) {
    	String rowid = "";
    	for (WebElement element : tableDatas) {
			if(testData.equals(element.getText())) {
				rowid = element.findElement(By.xpath("./..")).getAttribute("row-id");
				break;
			}
		}
    	return rowid;
	 ...
	 * 判断文本是不是和需求要求的文本一致
	 * **/
    public void isTextCorrect(String testData, List<WebElement> tableDatas) {
    	String[] expected = testData.split("=");
    	String actual = "True";
		if(tableDatas.size()==0 || "".equals(getRowId(expected[1], tableDatas))){
			actual = "Fail";
		}
		try {
     ...
     * @param tableDatas
     */
    public void doubleClick(String testData, List<WebElement> tableDatas) {
		// TODO Auto-generated method stub
    	if(tableDatas.size()==0) {
    		logger.info("The list of data is empty！");
    		Assert.fail("The list of data is empty！");
    	}
    	boolean doubleClickResult = false;
    	for (WebElement element : tableDatas) {
			if(testData.equals(element.getText())) {
