...
//		FramePageHelper.jumpInToFrame(seleniumUtil, FramePage.FP_FRAME_NAVBAR);// 再进入body
	...
	public static void typeVerificationCode(SeleniumUtil seleniumUtil, String VCode){
		logger.info("开始输入验证码");
		seleniumUtil.clear(LoginPage.LP_INPUT_VCODE);
		seleniumUtil.type(LoginPage.LP_INPUT_VCODE, VCode);
		logger.info("输入验证码信息完毕");
		// 点击登录按钮
		seleniumUtil.click(LoginPage.LP_BUTTON_LOGIN);
	}
